...
def convert_custom_namelist(names, field, match_table):
	...
	if field=='symbol':
		...
		conversion = match_table.reindex(names)[~(match_table.reindex(names)['Symbol'].isnull())]
		...
		# Return match table values that have matched symbol
		conversion = match_table.reindex(names)[~(match_table.reindex(names)['EntrezID'].isnull())]
		if conversion.shape[0]==0:
			return None
		else:
			# Return conversion with max score or None if no conversion
